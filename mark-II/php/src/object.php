...
class Object{
	public function __construct($json, $id = 0){
		...
		if(isset($json["nd_fields"]) && $json["nd_fields"]) {
			$i = count(Field::$_nd_fields);
			$field_name;
			while ($i--) {				
				$field_name = Field::$_nd_fields[$i]["name"];
				$this->_fields[$field_name] = new Field(Field::$_nd_fields[$i]);
			};
		};

		$i = count($json["fields"]);
		while ($i--) {
			$this->_fields[$json["fields"][$i]["name"]] = new Field($json["fields"][$i]);
		};
